...
module.exports = CoreObject.extend({
  init: function() {
    ...
    this.indexRevisionsPrefix = this.config.revisionsPrefix || '';
    this.activeIndexPrefix = this.config.indexPrefix || '';

    // Using `revisionsPrefix` to store revisions in a folder needs "indirect" as `indexMode`
    if (this.indexRevisionsPrefix.indexOf('/') >= 0 && this.indexMode !== 'indirect') {
      throw new Error("You need to set 'indirect' indexMode for using revisionsPrefix to store revisions in a folder");
    }
  },
  ...
  activate: function(revision) {
    var revisionWithPrefix = this.indexRevisionsPrefix.concat(revision);
  ...
  _printBucketRevisions: function(revisions) {
    ...
    var revisionsList = revisions.reduce(function(prev, current, index) {
      return prev + '\n\n' + (index + 1) + ') ' + current.replace(this.indexRevisionsPrefix, '');
    }.bind(this), '');
  ...
  _getBucketContents: function() {
    return new Promise(function(resolve, reject) {
      this.client.listObjects({
        ...
        Prefix: this.indexRevisionsPrefix
      }, function(err, data) {
        if (err) {
          reject(err);
        } else {
      ...
      return new Promise(function(resolve, reject) {
        ...
        .then(function(contents) {
          if (this.indexMode === 'direct') {
            ...
            var activeIndexKey = this.activeIndexPrefix + 'index';
            var params = this._getUploadParams(activeIndexKey, contents)
  ...
  _removeCurrentRevisionFromContents: function(data) {
    for (var i = 0, len = data.Contents.length; i < len; i++) {
      var fileName = data.Contents[i].Key.replace(this.indexRevisionsPrefix, '');
    ...
    return new Promise(function(resolve, reject) {
      ...
      .then(function(revisions) {
        if (revisions.indexOf(key) < 0) {
          var keyWithPrefix = this.indexRevisionsPrefix + key;
          var params = this._getUploadParams(keyWithPrefix, value);
          this.client.putObject(params, function(err, data) {
            if (err) {
